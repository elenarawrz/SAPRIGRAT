...
public class Administrador extends CustomComponent implements Formulario
{
					...
					else
					{
						if(tblListado.getValue() != null)
						{
							ConfirmDialog.show( event.getButton().getUI(),
												"Atención:",
												"Está a punto de eliminar un Administrador de Distrito y esta acción no se puede deshacer." +
												...
												new ConfirmDialog.Listener()
												{
													public void onClose(ConfirmDialog dialog)
													{
														if (dialog.isConfirmed())
														{
															...
															String status = datos.eliminarAdministrador(rowSel, curr);
															if(!status.isEmpty() && status.indexOf("err") == -1)
															{
																lblRowCount.setValue(u.llenarTabla(tblListado, datos.getAdministradores(tipoPersona), TipoUsuario.ADMINISTRADOR_DISTRITO.toString()));
																u.notificar("El administrador de distrito fue eliminado correctamente", null, u.MSJ_BANDEJA);
															}
															else
																if(status.indexOf("Tec") == -1)
																	u.notificar("El administrador de distrito no se pudo eliminar porque tiene supervisores técnicos asignados, si desea continuar elimine primero los supervisores e inténtelo de nuevo", null, u.MSJ_ADVERTENCIA);
																else
							...
							u.notificar("No ha seleccionado ningún administrador de distrito a eliminar", null, u.MSJ_ADVERTENCIA);
					}
				}
			};
	//endregion
	...
	public void inicializar()
	{
		...
		u.setupCombo(cmbEntidad, entidades, null, null, false, true);
		u.setupCombo(cmbNombreEnt, null, null, null, true, true);
