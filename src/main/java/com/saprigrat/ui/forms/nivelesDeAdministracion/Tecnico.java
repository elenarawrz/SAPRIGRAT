...
public class Tecnico extends CustomComponent implements Formulario
{
						...
						if(!curr.isEmpty() && curr.indexOf("err") == -1)
						{
							...
							if(curr.indexOf("Log") == -1)
								...
								if(curr.indexOf("LogRep") != -1)
									...
									Notification.show("Se guardó la información del técnico, pero sus credenciales para ingresar al sistema no. Comuníquese con el administrador.", Notification.Type.WARNING_MESSAGE);
					}
				}
			};
	...
	private ValueChangeListener llenarTablaListener = new ValueChangeListener()
			{
				...
				{
					u.llenarTabla(tblListado, datos.getTecnicos(((String)cmbResponsable.getValue()).split(" ")[0], tipoPersona));
				}
			};
	...
	public void inicializar()
	{
		...
		u.setupCombo(cmbFormacion, datos.getFormacion(), null, null, false);
	...
	private boolean validar()
	{
		if(cmbResponsable.getValue() == null)
			return notificar("Se requiere especificar el " + responsableDirecto[tipoPersona - 1], cmbResponsable);
	...
	private LinkedList<Object> llenarListaDeParametros()
	{
		...
		valores.add(u.fechaUtilToSQL(datNacimiento.getLocalDate().toDate()));
