...
public class GeneralesParcela extends CustomComponent implements Formulario
{
	...
	private ItemClickListener llenarFormListener = new ItemClickListener()
			{
				...
				public void itemClick(ItemClickEvent event)
				{
					...
					{
						idGeneralesParcela = Integer.parseInt(tblListado.getItemCaption(event.getItem()).split(" ")[0]);
	...
	private ValueChangeListener llenarTablaListener = new ValueChangeListener()
			{
				...
				{
					u.llenarTabla(tblListado, datos.getParcelas(((String)cmbResponsable.getValue()).split(" ")[0]));
				}
			};
	...
	public GeneralesParcela(Usuario user)
	{
		usuario = user;
		u = new Utilerias();
		datos = new Datos();
	...
	public void inicializar()
	{
		...
		u.setupCombo(cmbFAbast, datos.getAbastecimiento(), null, null, false);
		u.setupCombo(cmbTenencia, datos.getTenencia(), null, null, false);
		u.setupCombo(cmbOrgCuenca, datos.getOrgCuenca(), null, FilteringMode.CONTAINS, false);
		u.setupCombo(cmbDtoR, datos.getDtoRiego(), null, null, true);
...
//		};
	...
	private void setResponsableTecnico(int tipoAgregando)
	{
		...
		else
		{
			...
			if(cmbResponsable.size() == 0)
			{
				...
				Notification.show("Imposible registrar. Se requiere que exista en la base de datos al menos un productor a quien pueda ser asignado esta parcela.", Notification.Type.ERROR_MESSAGE);
			}
		}
	}
	...
	private AbsoluteLayout buildAbsLCoordenadas1() {
		...
		txtCoordY.setWidth("240px");
		txtCoordY.setHeight("25px");
		absLCoordenadas1.addComponent(txtCoordY, "top:28.0px;left:270.0px;");
