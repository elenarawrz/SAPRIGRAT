...
public class Riego extends CustomComponent implements Formulario, FormParcela
{
	...
	private boolean validar()
	{
		...
		if(cmbTipoRiego.getValue() == null)
			return u.notificar("Se requiere especificar el tipo de riego", cmbTipoRiego, u.MSJ_ERROR);
	...
	private boolean validarRiego()
	{
		...
		if(txtEficiencia.getValue().isEmpty() || !txtEficiencia.isValid())
			return u.notificar("Se requiere especificar un valor estríctamente numérico para la eficiencia", txtEficiencia, u.MSJ_ERROR);
	...
	private LinkedList<Object> getRiego()
	{
		...
		{
			row.add(new SimpleDateFormat(formatoFechaTblSH).format(datFechaRiego.getValue()));
			row.add("");
			row.add(0D);
			row.add(0);
			row.add(datRIndex == 2 ? Double.parseDouble(txtDatoDirecto.getValue()) : 0D);
			row.add(datRIndex == 1 ? Double.parseDouble(txtDatoDirecto.getValue()) : 0D);
		}
		row.add(3, txtEtapa.getValue());
		row.add(7, superficie);
		row.add(Double.parseDouble(txtLaminaCalc.getValue()));
		row.add(Double.parseDouble(txtEficiencia.getValue()));
		row.add(datRIndex);
	...
	private AbsoluteLayout buildAbsLRiego2() {
		...
		txtEficiencia.setWidth("165px");
		txtEficiencia.setHeight("25px");
		absLRiego2.addComponent(txtEficiencia, "top:18.0px;left:420.0px;");
		...
		absLRiego2.addComponent(lblEficiencia, "top:22.0px;left:590.0px;");
