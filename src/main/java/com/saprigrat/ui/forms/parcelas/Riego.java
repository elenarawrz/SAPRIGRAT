...
public class Riego extends CustomComponent implements Formulario
{
	...
	@AutoGenerated
	private VerticalLayout verLBotonesRiego;
	@AutoGenerated
				...
				public void itemClick(ItemClickEvent event)
				{
					if(event.isDoubleClick())
					{
						...
						if(!valores.isEmpty())
						{
							...
							txtLaminaAnt.setValue(valores.remove() + "");
	...
	private ValueChangeListener llenarTablaListener = new ValueChangeListener()
			{
				...
				{
					u.llenarTabla(tblListado, datos.getRiegos(((String)cmbResponsable.getValue()).split(" ")[0]));
				}
			};
	...
	public void inicializar()
	{
		...
		u.setupCombo(cmbTipoRiego, datos.getTipoRiego(), null, null, false);
		cmbProductor.addValueChangeListener(llenarParcelasListener);
		//Calendarios
	...
	private void mostrarPanel(boolean estado)
	{
		...
		if(estado)
		{
			absLRiego.setHeight("115px");
			btnCancelar.setClickShortcut(KeyCode.C, ModifierKey.ALT);
		}
		else
		{
			absLRiego.setHeight("30px");
			btnCancelar.removeClickShortcut();
		}
	}
	...
	private boolean validarRiego()
	{
		if(txtEtapa.getValue() == null)
			return notificar("Se requiere especificar la etapa fenológica", txtEtapa);
		...
		if(txtSuperficie.getValue().isEmpty() || !txtSuperficie.isValid())
			...
			if(Double.parseDouble(txtSuperficie.getValue()) == 0)
				return notificar("La superficie no puede ser igual a 0", txtSuperficie);
	...
	private LinkedList<Object> getArreglosRiego()
	{
		...
		Long[] arrGasto = new Long[rows],
			   arrVolumen = new Long[rows],
			   arrSuperficie = new Long[rows],
			   arrLamApl = new Long[rows],
			   arrLamCalc = new Long[rows],
			   arrEficiencia = new Long[rows];
		...
		for(int i = 0; i < rows; i++)
		{
			...
			arrGasto[i] = (Long) riego.pop();
			arrTiempo[i] = (Integer) riego.pop();
			arrVolumen[i] = (Long) riego.pop();
			arrSuperficie[i] = (Long) riego.pop();
			arrLamApl[i] = (Long) riego.pop();
			arrLamCalc[i] = (Long) riego.pop();
			arrEficiencia[i] = (Long) riego.pop();
		}
	...
	{
		Long gasto = Long.parseLong(txtGasto.getValue());
		Integer tiempo = (int)((datFin.getValue().getTime() - datInicio.getValue().getTime()) / 3600000);
		Long volumen = gasto * 3600 * tiempo * 1000; //Gasto[L/s]*3600:[L/h] -> Gasto[L/h]*Tiempo[h]:[L] -> [L]*1000 = Volumen[m3]
		Long superficie = Long.parseLong(txtSuperficie.getValue());
		Long laminaApl = (volumen / (superficie * 10000)) * 100; //Superficie[ha]*10000:[m2] -> Volumen[m3]/Superficie[m2]:[m] -> [m]*100 = LaminaApl[cm]
		...
		row.add(superficie);
		row.add(laminaApl);
		row.add(Long.parseLong(txtLaminaCalc.getValue()));
		row.add(Long.parseLong(txtEficiencia.getValue()));
	...
	private AbsoluteLayout buildAbsLRiego() {
		...
		// verLBotonesRiego
		verLBotonesRiego = buildVerLBotonesRiego();
		absLRiego.addComponent(verLBotonesRiego, "right:0.0px;bottom:0.0px;");
	...
	private AbsoluteLayout buildAbsLRiego1() {
		...
		datInicio.setWidth("150px");
		datInicio.setHeight("25px");
		absLRiego1.addComponent(datInicio, "top:28.0px;left:370.0px;");
		
		// datFin
		datFin = new DateField();
		datFin.setCaption("Fecha y Hora de Fin");
		datFin.setImmediate(false);
		datFin.setWidth("150px");
		datFin.setHeight("25px");
		absLRiego1.addComponent(datFin, "top:28.0px;left:540.0px;");
	...
	private AbsoluteLayout buildAbsLRiego2() {
		...
		txtGasto.setWidth("135px");
		txtGasto.setHeight("25px");
		absLRiego2.addComponent(txtGasto, "top:18.0px;left:10.0px;");
		...
		absLRiego2.addComponent(lblGasto, "top:22.0px;left:150.0px;");
		
		// txtSuperficie
		txtSuperficie = new TextField();
		txtSuperficie.setCaption("Superficie Regada");
		txtSuperficie.setImmediate(true);
		txtSuperficie.setWidth("135px");
		txtSuperficie.setHeight("25px");
		absLRiego2.addComponent(txtSuperficie, "top:18.0px;left:185.0px;");
		...
		absLRiego2.addComponent(lblSuperficie, "top:22.0px;left:325.0px;");
		
		// txtLaminaCalc
		txtLaminaCalc = new TextField();
		txtLaminaCalc.setCaption("Lamina Calculada");
		txtLaminaCalc.setImmediate(true);
		txtLaminaCalc.setWidth("135px");
		txtLaminaCalc.setHeight("25px");
		absLRiego2.addComponent(txtLaminaCalc, "top:18.0px;left:355.0px;");
		...
		absLRiego2.addComponent(lblLaminaCalc, "top:22.0px;left:495.0px;");
		
		// txtEficiencia
		txtEficiencia = new TextField();
		txtEficiencia.setCaption("Eficiencia Parcelaria");
		txtEficiencia.setImmediate(true);
		txtEficiencia.setWidth("140px");
		txtEficiencia.setHeight("25px");
		absLRiego2.addComponent(txtEficiencia, "top:18.0px;left:530.0px;");
		...
		absLRiego2.addComponent(lblEficiencia, "top:22.0px;left:675.0px;");
		
		return absLRiego2;
	}

	@AutoGenerated
	private VerticalLayout buildVerLBotonesRiego() {
		// common part: create layout
		verLBotonesRiego = new VerticalLayout();
		verLBotonesRiego.setImmediate(false);
		verLBotonesRiego.setWidth("-1px");
		verLBotonesRiego.setHeight("-1px");
		verLBotonesRiego.setMargin(false);
		verLBotonesRiego.setSpacing(true);
		...
		verLBotonesRiego.addComponent(btnCancelar);
		...
		verLBotonesRiego.addComponent(btnAdd);
		
		return verLBotonesRiego;
	}
